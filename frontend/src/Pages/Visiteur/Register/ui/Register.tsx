...
prenom: z.string().min(1, "Le prénom est requis").regex(/^[A-Za-zÀ-ÿ\s'-]+$/, "Le prénom ne doit contenir que des lettres"),
    ...
    login: z
      ...
      .regex(/^\S+$/, "Le login ne doit pas contenir d'espace"),
    password: z.string().min(6, "Mot de passe trop court"),
    confirmPassword: z.string().min(6, "Confirmation requise"),
    numTel: z
      .string()
      .regex(/^[0-9]{10}$/, "Numéro invalide (10 chiffres sans +)"),
...
const Register: React.FC = () => {
 ...
 const onSubmit = (data: RegisterFormValues) => {
   ...
   registerMutation.mutate({
     ...
     abonnementInscritId: data.abonnementInscritId,
   });
  ...
  return (
    <div 
      className="login-page min-h-screen bg-cover bg-center" 
          ...
          <div className="w-full lg:w-3/4 flex justify-center">
            <div className="glass-effect rounded-xl p-8 w-full max-w-4xl shadow-lg">
              ...
              <div className="flex flex-col space-y-4 mb-6">
                <Button variant="social" size="full" className="bg-white text-foreground hover:bg-gray-100">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 gap-6">
                  ...
                  <div className="flex flex-col items-center gap-4 mt-4">
                    ...
                    >
                      {registerMutation.isLoading ? "Chargement..." : "S'inscrire"}
                    </Button>

                    {registerMutation.isError && typeof registerMutation.error?.message === "string" && (
                      <Alert variant="destructive" className="animate-fadeIn">{registerMutation.error.message}</Alert>
                    )}
                    {registerMutation.isSuccess && (
                      <Alert variant="success" className="animate-fadeIn">✅ Inscription réussie !</Alert>
                    )}

                    <a href="/login" className="text-white text-sm hover:text-brand-orange hover:underline transition-colors">
                      J'ai déjà un compte !
                    </a>
